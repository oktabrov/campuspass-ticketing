...
class InvalidBookingError(ValidationError):
    def __init__(self, field, value, reason):
        ...
        self.value = value
        self.reason = reason
        super().__init__(f"Invalid booking for '{field}': {reason} (received: {value})")
...
class SoldOutError(AvailabilityError):
    def __init__(self, event_name, tier_name, requested, remaining):
        ...
        super().__init__(f"Requested {requested} {tier_name} ticket(s) for {event_name}, only {remaining} remaining.")
...
class DuplicateBookingError(AvailabilityError):
    def __init__(self, email, event_name):
        self.email = email
        self.event_name = event_name
        super().__init__(f"{email} has already booked tickets for {event_name}.")
...
class PaymentDeclinedError(PaymentError):
    def __init__(self, gateway_name, reason):
        ...
        self.reason = reason
        super().__init__(f"Payment was declined by {gateway_name}: {reason}.")
